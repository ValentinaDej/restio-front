...
import { getSelectedOrders } from 'store/customer/orders/selectors';
...
export const OrdersList = ({ orders = mockOrders }) => {
  const { payOrders, billOrders } = useSelector(getSelectedOrders);
  const dispatch = useDispatch();

  const selectPayOrder = useCallback(
    (id) => {
      dispatch(customerOrdersActions.selectPayOrder(id));
    },
    [dispatch]
  );
  ...
  const renderOrder = (order) => (
    <OrderCard
      key={order._id}
      {...order}
      checkedPay={payOrders?.includes(order._id)}
      checkedBill={billOrders?.includes(order._id)}
